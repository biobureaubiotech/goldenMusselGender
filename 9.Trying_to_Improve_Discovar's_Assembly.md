...
#### 02/20/2019  
#### Switching to another gap-filling algorithm  
Gap2Seq was going to take too long to complete according to the execution status. Taking too long to gap-fill large genomes is actually a common problem with gap-filling algorithms, that are usually more suitable for bacterial genomes. A promising alternative algorithm is the Sealer, part of the Abyss assembly pipeline. According to its [paper](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4515008/), Sealer was capable of gap-filling the human genome in 29 hours using a 12 cores, 48G RAM machine. In order to test Sealer for gap-filling our LF2 dataset, we have run the command: 
